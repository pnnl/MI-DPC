...
class MIMPC_policy():
    ...
    Optimized MPC with Pyomo:
      ...
      - Optional persistent solver for better performance
    """
    def __init__(self,
                 nsteps,
                 ...
                 use_persistent=True,             # uses 'gurobi_persistent' when solver='gurobi'
        ...
        self.use_persistent = (use_persistent and solver.lower() == "gurobi")
        ...
        # (Adjust as needed for your environment/solver)
        try:
        ...
        try:
            ...
            self.solver.options['MIPGap'] = 1e-3
        except Exception:
            pass
        ...
        # Attach instance to persistent solver (once)
        if self.use_persistent:
    ...
    def _build_model(self):
        ...
        if self.nsteps >= 2:
            ...
            # Empty set to keep constraints well-defined if nsteps==1
            m.tm1 = RangeSet(0, -1)
        ...
        m.Ts = Param(m.t, mutable=True, initialize=lambda m, t: self.Ts_default if self.Ts_default is not None else 1.0)
        ...
        if self.ocp_formulation == 0:
            ...
            # Keep your exact discretization here if using a nonlinear solver (e.g., IPOPT)
            # but note: Gurobi cannot handle exp() directly (nonlinear).
        ...
        def _P_pump_fn(m, t, i):
            ...
            return m.P_pump[t, i] == init.gamma * m.active_flow[t, i] ** 2
        ...
        def _obj_expr(m):
            ...
            return quicksum(
                1000. * (quicksum(m.Q_delivered[t, i] for i in m.i) - m.load_demand[t]) ** 2
                + quicksum(m.P_chiller[t, i] + m.P_pump[t, i] + 1.0 * m.integer_smooth[t, i] + 1.0 * m.flow_smooth[t, i] for i in m.i)
                for t in m.t
            )
    ...
    def _warm_start_defaults(self):
        ...
        This helps warmstart=True (and persistent solvers) do their job.
        """
        m = self.model
    ...
    def __call__(self, T_supply=None, T_return=None, load=None, filtered_load=None, Ts=None):
        ...
        if self.use_persistent:
            self.solver.set_instance(self.model)
            results = self.solver.solve(m, warmstart=True)
        else:
            results = self.solver.solve(m, tee=False, symbolic_solver_labels=False, warmstart=False)
...
if __name__ == "__main__":
    ...
    policy = MIMPC_policy(
        nsteps=nsteps,
        ocp_formulation=0,        # Euler (MIQP-friendly)
        verbose=False,
        solver="gurobi",
        use_persistent=False,      # fastest with Gurobi
    ...
    result2 = policy(
        T_supply=T_supply,
        T_return=T_return,
        load=load * 1.05,         # slightly different load
        Ts=300.0
    )
